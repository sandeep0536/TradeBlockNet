...
app.post('/sendmail', ({ body }, res) => {
    try {
        var transporter = nodemailer.createTransport({
            service: 'gmail',
            auth: {
                user: 'dltdeveloper6234@gmail.com',
                pass: 'dlt@6234'
            },
            tls: {
        ...
        var mailOptions = {
            from: 'dltdeveloper6234@gmail.com',
            to: body.to,
...
/**
 * Api will call when user subscribe any paid plan 
 */
app.post('/payment', async ({ body }, res) => {
    if (checkConnection) {
        const upgradePlan = false;
        const changePlan = false;
        con.query(`SELECT plan,u.sub_end_date,u.purchase_date from plans_table p JOIN user_table u ON p.id=u.sub_type where email = '${body.email}'`, async function (err, result) {
            if (err) console.log(err);
            if (Object.keys(result).length < 1) {
                await callStripe(body.plan, body.id, body.amount, body.email, "", "", res)
            }
            else if (result[0].plan == body.plan) {
                await callStripe(body.plan, body.id, body.amount, body.email, "upgrade", "", res)
            } else {
                await callStripe(body.plan, body.id, body.amount, body.email, "", "change", res)
            }
        });
    }
})

async function callStripe(plan, id, amount, email, upgrade, change, res) {
    const session = await stripe.checkout.sessions.create({
        ...
        mode: "payment",
        success_url: `https://www.nextfileshare.com/success?planname=${plan}&plan=${id}&email=${email}&upgrade=${upgrade}&change=${change}`,
        cancel_url: "https://www.nextfileshare.com/subscription",
    });
                    ...
                    if (err) console.log(err);
                        ...
                        con.query(new_sql, function (err, result) {
                            if (err) console.log(err)
                                ...
                                console.log("your plan successfully upgraded")
                                res.redirect(`https://www.nextfileshare.com/subscription`)
                            }
                        })
                    }
                });
