...
import { CONTRACT_ADDRESS, CONTRACT_ABI, COMMON } from 'ServerConfig';
...
export default function EInvoicing(props) {
    ...
    const getFilehash = async (hash, i) => {
        ...
        const hashData = await getFileMetadata();
        if (hashData == undefined)
            setStatus(false)
        const filehash = JSON.parse(hashData);
        setTimeout(function () {
            try {
    ...
    const setMetaData = async (decPrivate, name, type, address, hash) => {
        try {
            ...
            return web3.eth.sendSignedTransaction('0x' + serializedTx.toString('hex')).on('receipt', receipt => {
                count++;
                console.log(receipt)
                StoreFiles(hash, projectname, address)
    ...
    return (
        <div className="m-sm-30">
            <div className="mb-sm-30">
                <Breadcrumb
                    routeSegments={[
                        { name: <div id="upload-crumb">{projectname}</div>, path: '/e-invoicing', pathname: projectname },
                        { name: 'E invoicing' },
                    ]}
                />
            </div>
